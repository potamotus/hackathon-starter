		...
		try {
			...
			eventSource.onmessage = (e) => {
				try {
					...
					if (show) {
						getMemories(localStorage.token).then((m) => (memories = m));
					}
				} catch {}
			};
			eventSource.onerror = () => {
...
<Modal size="lg" bind:show>
	<div>
		...
		<div class="flex flex-col w-full px-5 pb-5 dark:text-gray-200">
			<div class="flex flex-col w-full h-[28rem] max-h-screen rounded-xl mb-4 mt-1">
												...
												<div class="flex items-center gap-2">
													{#if memory.memory_type && TYPE_COLORS[memory.memory_type]}
														<span
															...
															style="background:{TYPE_COLORS[memory.memory_type]}"
														/>
													{/if}
													<div class="min-w-0">
													...
													<Tooltip content="Delete">
														<button
															...
															on:click={async () => {
																...
																if (res) { toast.success($i18n.t('Memory deleted successfully')); memories = await getMemories(localStorage.token); }
															}}
														>
															<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
...
<AddMemoryModal bind:show={showAddMemoryModal} on:save={async () => { memories = await getMemories(localStorage.token); }} />
